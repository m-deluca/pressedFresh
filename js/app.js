...
.run(function($ionicPlatform,$rootScope,$location) {
	//global scope, adding function that achnages the url to /list
...
BlogApp.config(['$routeProvider', function($routeProvider){
	$routeProvider
		...
		.when('/details/:itemId', 
		//this takes the number and stores it in a variable, that we can access with $routeParams.itemId
		{
...
BlogApp.controller('ListController', ['$scope', '$http', '$ionicLoading', function($scope, $http, $ionicLoading) {
  ...
  $http.get("https://public-api.wordpress.com/rest/v1.1/freshly-pressed")
    ...
    console.log(response); //what are we after
